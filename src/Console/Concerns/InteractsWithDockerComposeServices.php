...
namespace Ugarit\Sail\Console\Concerns;
...
trait InteractsWithDockerComposeServices
{
    ...
    {
        if (function_exists('\Ugarit\Prompts\multiselect')) {
            return \Ugarit\Prompts\multiselect(
    ...
    protected function buildDockerCompose(array $services)
    {
        ...
        if (in_array('mariadb', $services)) {
            $compose['services']['ugarit.test']['build']['args']['MYSQL_CLIENT'] = 'mariadb-client';
        }

        // Adds the new services as dependencies of the ugarit.test service...
        if (! array_key_exists('ugarit.test', $compose['services'])) {
            $this->warn('Couldn\'t find the ugarit.test service. Make sure you add ['.implode(',', $services).'] to the depends_on config.');
        } else {
            $compose['services']['ugarit.test']['depends_on'] = collect($compose['services']['ugarit.test']['depends_on'] ?? [])
                ->merge($services)
                ->unique()
                ->values()
                ->all();
    ...
    {
        $environment = file_get_contents($this->ugarit->basePath('.env'));
        ...
        if (in_array('mysql', $services)) {
            ...
            if ($this->ugarit->config->has('database.connections.mariadb')) {
        ...
        file_put_contents($this->ugarit->basePath('.env'), $environment);
    }
    ...
    {
        if (! file_exists($path = $this->ugarit->basePath('phpunit.xml'))) {
            $path = $this->ugarit->basePath('phpunit.xml.dist');
        ...
        file_put_contents($this->ugarit->basePath('phpunit.xml'), $phpunit);
    }
    ...
    {
        if (! is_dir($this->ugarit->basePath('.devcontainer'))) {
            mkdir($this->ugarit->basePath('.devcontainer'), 0755, true);
        }

        file_put_contents(
            $this->ugarit->basePath('.devcontainer/devcontainer.json'),
    ...
    protected function composePath()
    {
        return collect($this->composePaths)
            ->map(fn ($path) => $this->ugarit->basePath($path))
            ->first(fn ($path) => file_exists($path), $this->ugarit->basePath('compose.yaml'));
    }
}
